...
class DecisionEngine:
    ...
    def process_all_pending(
        self, ticker: str | None = None, verbose: bool = True
    ) -> ProcessingReport:
        """
        ...
        report = ProcessingReport(
            total=len(proposals), analyzed=0, auto_decided=0, escalated=0, errors=[]
        )
    ...
    def _should_escalate(self, result: AnalysisResult) -> bool:
        ...
        # Always escalate certain high-stakes types
        if ptype in {t.value for t in ALWAYS_ESCALATE_TYPES}:
            return True

        # Executive compensation: escalate if important enough
        if (
    ...
    def get_review_queue(self) -> list:
        """Return proposals that need human input, ordered by importance DESC."""
    ...
    def apply_user_vote(
        ...
        """
        Record the user's vote override. Optionally trigger preference learning.
        """
        # Fetch current decision info for learning
